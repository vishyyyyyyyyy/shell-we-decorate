...
func _input_event(viewport, event, shape_idx):
	...
	and event.button_index == MOUSE_BUTTON_LEFT:
		if shape_idx == 0 and Global.pearls_earned >= 100:
			...
			return
		if shape_idx == 1 and Global.pearls_earned >= 200:
			...
			return
		if shape_idx == 2 and Global.pearls_earned >= 300:
			...
			return
		if shape_idx == 3 and Global.pearls_earned >= 400:
